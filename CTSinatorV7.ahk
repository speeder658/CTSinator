...
^#!g::
	...
	else if ticktype := "Catalog Task"
	{
		...
		Send, Requested for
		Sleep, 300
		PixelSearch, Px, Py, 0, 120, 341, 341, 0xff9632, , Fast RGB
		if ErrorLevel
			{
			PixelSearch, Px, Py, 0, 120, 341, 341, 0xffff00, , Fast RGB
			MouseMove, %Px%, %Py%
			}
		else
			MouseMove, %Px%, %Py%
		...
		isSoftwareRequest := InStr(item, "Software requests")
		isDirShares := InStr(item, "Directory\Shares")

		if isSoftwareRequest = 1
		{
			Send, {WheelUp 3}
			Send, ^f  ;SEARCHWINDOW_START
			Send, {BackSpace}
			...
			Send, Requested for
			Sleep, 300
			PixelSearch, Px, Py, 0, 120, 340, 660, 0xff9632, , Fast RGB
			MouseMove, Px, Py
			Send, {Esc}
			Send, {LButton}
			Sleep, 1000
			...
			if isEmail = 0
				{
					MsgBox, email not correct, copy manually
					ClipWait
				}
			...
			{
				Msgbox, manual mode activated, copy the software name
				clipboard :=
				ClipWait
				softname := clipboard
				}
		...
		else if isSoftwareRequest = 0
		{
			MsgBox, Not a software request - manual mode will activate
			if isDirShares = 1
			{
				MsgBox, it's an access issue / request, check the log
			}
			InputBox, email, email, paste in the email, , , , , , , , first.last@woodplc.com
			InputBox, desc, description, paste in the ticket description, , , , , , , , access to whatever
